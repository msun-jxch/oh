...
logging.info('读取初始数据')
table_df = DataFrame([])
df = pd.read_excel(r'../res/体检项目关联.xlsx', sheet_name='必检项目')
df = format_str(df)

logging.info('写入清洗数据')
with pd.ExcelWriter(tmp_path, engine='openpyxl', mode='w') as writer:
    df.to_excel(writer, '必检项目', index=False, header=True)
...
with pd.ExcelWriter(tmp_path, engine='openpyxl', mode='a') as writer:
    table_df.to_excel(writer, 'hazard_vs_bjxm', index=False, header=True)
