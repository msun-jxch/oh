...
tmp_path = r'../res/temp_hazard_vs_xjxm_df.xlsx'


def format_str(the_df):
    the_df['危害因素'] = the_df['危害因素'].map(lambda name: name.replace('(', '（').replace(')', '）').replace(' ', ''))
    the_df['选检项目'] = the_df['选检项目'].map(
        lambda name: name.replace('(', '（').replace(')', '）').replace(' ', '').split('（见附录')[0].strip(','))
    return the_df


logging.info('读取初始数据')
table_df = DataFrame([])
df = pd.read_excel(r'../res/体检项目关联.xlsx', sheet_name='选检项目')
df = format_str(df)
...
table_df['order_name'] = df['选检项目'].map(lambda name: dict_oh_order_extend_util.get_dict_order_name(name))
...
with pd.ExcelWriter(tmp_path, engine='openpyxl', mode='a') as writer:
    table_df.to_excel(writer, 'hazard_vs_xjxm', index=False, header=True)
