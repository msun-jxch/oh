...
df.rename(columns={'3.12危害因素编码': 'hazard_factor_code_upload'}, inplace=True)
df.rename(columns={'Unnamed: 2': 'hazard_factor_name'}, inplace=True)
df.drop(columns=['Unnamed: 0'], inplace=True)
df.drop(axis=0, index=0, inplace=True)

df = df.query('hazard_factor_code_upload>100')

df.reset_index(drop=True, inplace=True)
df['hazard_factor_id'] = df.index
df.set_index(['hazard_factor_id'], inplace=True)
df['hazard_type_id'] = df['hazard_factor_code_upload'] // 10000
